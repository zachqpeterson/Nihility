...
{
    if (!SelectPhysicalDevice(rendererState))
    {
        return false;
    }
    ...
    if (!presentSharesGraphicsQueue)
    {
        indices[index++] = rendererState->device->presentQueueIndex;
    }
    if (!transferSharesGraphicsQueue)
    {
        indices[index++] = rendererState->device->transferQueueIndex;
    }
    ...
    {
        extensionNames.Push("VK_KHR_portability_subset");
    }
    ...
    VkCheck(vkCreateDevice(
        rendererState->device->physicalDevice,
        &deviceCreateInfo,
    ...
    vkGetDeviceQueue(
        rendererState->device->logicalDevice,
        rendererState->device->graphicsQueueIndex,
        0,
        &rendererState->device->graphicsQueue);

    vkGetDeviceQueue(
        rendererState->device->logicalDevice,
        rendererState->device->presentQueueIndex,
        0,
        &rendererState->device->presentQueue);

    vkGetDeviceQueue(
        rendererState->device->logicalDevice,
        rendererState->device->transferQueueIndex,
        0,
        &rendererState->device->transferQueue);
    ...
    VkCheck(vkCreateCommandPool(
        ...
        &poolCreateInfo,
        rendererState->allocator,
        &rendererState->device->graphicsCommandPool));
...
void VulkanDevice::Destroy(RendererState* rendererState)
{
    ...
    {
        Memory::Free(
            rendererState->device->swapchainSupport.formats,
            sizeof(VkSurfaceFormatKHR) * rendererState->device->swapchainSupport.formatCount,
            MEMORY_TAG_RENDERER);
    ...
    {
        Memory::Free(
            rendererState->device->swapchainSupport.presentModes,
            sizeof(VkPresentModeKHR) * rendererState->device->swapchainSupport.presentModeCount,
            MEMORY_TAG_RENDERER);
    ...
    Memory::ZeroMemory(
        &rendererState->device->swapchainSupport.capabilities,
        sizeof(rendererState->device->swapchainSupport.capabilities));
...
bool VulkanDevice::SelectPhysicalDevice(RendererState* rendererState)
{
    ...
    for (U32 i = 0; i < physicalDeviceCount; ++i)
    {
        ...
        LOG_INFO("Evaluating device: '%s', index %u.", properties.deviceName, i);
        ...
        bool result = physicalDeviceMeetsRequirements(
            physicalDevices[i],
            rendererState->surface,
            &properties,
            &features,
            &requirements,
            &queueInfo,
            &rendererState->device->swapchainSupport);
        ...
        if (requirements->deviceExtensionNames.Size())
        {
            ...
            if (availableExtensionCount != 0)
            {
                ...
                for (U32 i = 0; i < requiredExtensionCount; ++i)
                {
                    ...
                    if (!found)
                    {
                        LOG_INFO("Required extension not found: '%s', skipping device->", requirements->deviceExtensionNames[i]);
                        Memory::Free(availableExtensions, sizeof(VkExtensionProperties) * availableExtensionCount, MEMORY_TAG_RENDERER);
                        return false;
                    }
                }
