...
HashTable<String, Font*> Resources::fonts;
...
bool Resources::Initialize()
{
	...
	LoadMaterial("Entity.mat");
	LoadMaterial("Liquid.mat");
	LoadMaterial("UI.mat");
	LoadMaterial("Text.mat");
...
Font* Resources::LoadFont(const String& name)
{
	if (name.Blank()) { Logger::Error("Font name can not be blank or nullptr!"); return nullptr; }
	...
	if (file.Open(path, FILE_MODE_READ, true))
	{
		font = (Font*)Memory::Allocate(sizeof(Font), MEMORY_TAG_RESOURCE);
		font->name = name;

		font->texture = LoadTexture(name + String{ ".bmp" });

		String line;
		file.ReadLine(line); // Header junk
		file.ReadLine(line); //
		file.ReadLine(line); //
		file.ReadLine(line); //
		...
		{
			I64 index;
			index = line.IndexOf('=') + 1; //ID
			U8 id = line.SubString(index, 3).ToU8();
...
void Resources::DestroyFont(Font* font)
{
	font->name.Destroy();

	Memory::Free(font, sizeof(Font), MEMORY_TAG_RESOURCE);
}