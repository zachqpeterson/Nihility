...
Controller::Controller(void* handle) : dHandle{ handle }, inputReportProtocol{ nullptr }
{
	...
	if (!(len = GetRawInputDeviceInfoA(dHandle, RIDI_DEVICENAME, path.Data(), &len))) { Logger::Trace("Failed to get path, skipping..."); return; }
	path.Resize(len);
...
void Controller::Destroy()
{
	...
	if (openHandle) { CloseHandle(ntHandle); openHandle = false; } //TODO: Sometimes crashes here...
}
