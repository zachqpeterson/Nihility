...
{
	if (!entity) { Logger::Error("Can't Add Nullptr Entity To Scene!"); return; }
	...
	if (entity->model)
	{
		...
		for (MeshInstance& instance : entity->model->meshes)
		{
			U64 handle = *handles.GetRequest(instance.mesh->handle, meshDraws.Size());
