...
constexpr Vector2 operator*(const Vector2& v, const Transform2D& t) { return v * t.rotation + t.position; }
constexpr Vector2 operator^(const Vector2& v, const Transform2D& t) { return (v ^ t.rotation) + t.position; }
constexpr Vector2& operator*=(Vector2& v, const Transform2D& t) { v *= t.rotation; v += t.position; return v; }
constexpr Vector2& operator^=(Vector2& v, const Transform2D& t) { (v ^= t.rotation); v += t.position; return v; }

template <class Type> inline constexpr bool IsFloatVectorType = AnyOf<RemoveQuals<Type>, Vector2, Vector3, Vector4>;
template <class Type> inline constexpr bool IsIntVectorType = AnyOf<RemoveQuals<Type>, Vector2Int, Vector3Int, Vector4Int>;
template <class Type> inline constexpr bool IsVectorType = AnyOf<RemoveQuals<Type>, Vector2, Vector3, Vector4, Vector2Int, Vector3Int, Vector4Int>;
template <class Type> inline constexpr bool IsMatrixType = AnyOf<RemoveQuals<Type>, Matrix2, Matrix3, Matrix4>;
template <class Type> inline constexpr bool IsQuaternionType = AnyOf<RemoveQuals<Type>, Quaternion2, Quaternion3>;
...
template <class Type> inline constexpr bool IsSplineType = AnyOf<RemoveQuals<Type>, BezierSpline, CatmullRomSpline, CardinalSpline, BSpline, HermiteSpline>;
template <class Type> concept SplineType = IsSplineType<Type>;