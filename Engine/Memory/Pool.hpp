...
template<class Type, U64 Count>
inline void Pool<Type, Count>::FreeResources()
{
	lastFree = 0;
...
template<class Type, U64 Count>
inline U64 Pool<Type, Count>::ObtainResource()
{
...
template<class Type, U64 Count>
inline void Pool<Type, Count>::ReleaseResource(U64 handle)
{
	// TODO: add bits for checking if resource is alive and use bitmasks.
...
template<class Type, U64 Count>
inline Type* Pool<Type, Count>::GetResource(U64 handle)
{
...
template<class Type, U64 Count>
inline const Type* Pool<Type, Count>::GetResource(U64 handle) const
{
	if (handle != U64_MAX) { return &memory[handle * ResourceSize]; }
