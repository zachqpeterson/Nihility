...
export template<class Type> requires (!IsVoid<Type>)
inline Type* Copy(Type* dst, const Type* src, U64 count)
{
	if (dst == src) { return dst; }

	if constexpr (IsNonPrimitive<Type> && IsCopyConstructible<Type>)
	{
		Type* dest = dst;
...
inline Type* Move(Type* dst, Type* src, U64 count)
{
	...
	if constexpr (IsNonPrimitive<Type> && IsMoveConstructible<Type>)
	{
		...
		if (dst > src && dst < src + count)
		{
			...
			while (count--) { new (dst--) Type(Move(*src--)); }
		}
		else
		{
			while (count--) { new (dst++) Type(Move(*src++)); }
	...
	else if constexpr (IsNonPrimitive<Type> && IsCopyConstructible<Type>)
	{
		Type* dest = dst;
		...
		if (dst > src && dst < src + count)
		{
			...
			{
				new (dst--) Type(*src);
				if constexpr (IsDestructible<Type>) { src->~Type(); }
				--src;
			}
		}
		else
