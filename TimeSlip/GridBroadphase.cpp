...
bool GridBroadphase::Query(PhysicsObject2D* obj, List<Contact2D>& contacts)
{
	...
	Vector2Int checkSize;
	
	if (move.x < 0.0f)
	{
		start.x -= extents.x;
		target.x = Math::Round(start.x) - 0.5f; //TODO: The tile size could be something other than 1, so use (tileSize * 0.5f)
		checkTile.x = (I32)(target.x - 0.5f);
		checkSize.x = (I32)(size.x - 0.001f) + 1;
		step.x = -1.0f;
	}
	else
	{
		start.x += extents.x;
		target.x = Math::Round(start.x - 0.001f) + 0.5f;
		checkTile.x = (I32)(target.x + 0.5f);
		checkSize.x = -(I32)(size.x - 0.001f) - 1;
		step.x = 1.0f;
	}
	...
	{
		start.y -= extents.y;
		target.y = Math::Round(start.y) - 0.5f;
		checkTile.y = (I32)(target.y - 0.5f);
		checkSize.y = (I32)(size.y - 0.001f) + 1;
		step.y = -1.0f;
	}
	else
	{
		start.y += extents.y;
		target.y = Math::Round(start.y - 0.001f) + 0.5f;
		checkTile.y = (I32)(target.y + 0.5f);
		checkSize.y = -(I32)(size.y - 0.001f) - 1;
		step.y = 1.0f;
	}
